<template>
  <div>
    ...
    <el-table :data="docs" border stripe v-loading="loading" @row-click="row => router.push(`/drawings/${row.id}`)" style="cursor:pointer">
      <el-table-column label="预览" width="120">
        <template #default="{ row }">
          <el-image v-if="isImage(row.mime_type)" :src="`/api/download/${encodeURIComponent(row.order_no)}/${encodeURIComponent(row.category)}/${encodeURIComponent(row.filename)}`" style="width:80px;height:60px;object-fit:cover;border-radius:4px" fit="cover" />
    ...
    <el-dialog v-model="uploadVisible" title="上传图纸" width="500px">
      <el-form label-width="80px">
        <el-form-item label="订单"><el-select v-model="uploadOrderNo" placeholder="选择订单" filterable style="width:100%"><el-option v-for="o in orders" :key="o.order_no" :label="`${o.order_no} ${o.product_name}`" :value="o.order_no" /></el-select></el-form-item>
        <el-form-item label="分类"><el-select v-model="uploadCat" placeholder="选择分类" style="width:100%"><el-option v-for="c in cats" :key="c" :label="c" :value="c" /></el-select></el-form-item>
        <el-form-item label="文件"><el-upload :auto-upload="false" :on-change="onFileChange" :limit="1" accept="image/*"><el-button>选择图片</el-button></el-upload></el-form-item>
...
const docs = ref([]); const orders = ref([]); const loading = ref(false);
const filterOrder = ref(null); const filterCat = ref('');
const uploadVisible = ref(false); const uploadOrderNo = ref(''); const uploadCat = ref('图纸');
const uploadFile = ref(null); const uploading = ref(false);
const cats = ['分流板图', '零件图', '精雕图', '线切割图', '图纸'];

onMounted(async () => { try { orders.value = (await api.get('/orders')).data.data; } catch {} fetchDocs(); });
