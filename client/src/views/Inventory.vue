  ...
  <div>
    ...
    <div class="bg-white dark:bg-industrial-800 border border-slate-200 dark:border-industrial-border rounded-xl p-4 mb-6 flex justify-between items-center shadow-sm">
      <div class="flex items-center gap-3">
        <el-input v-model="searchKeyword" placeholder="搜索物料名称或规格" clearable style="width: 280px" @clear="handleSearch" @keyup.enter="handleSearch">
    ...
    <div class="bg-white dark:bg-industrial-800 border border-slate-200 dark:border-industrial-border rounded-xl overflow-hidden shadow-md p-4">
      ...
      <div class="mt-4 flex justify-end">
        <el-pagination background layout="total, sizes, prev, pager, next" :total="total" v-model:current-page="page" v-model:page-size="limit" :page-sizes="[10, 20, 50]" @current-change="fetchItems" @size-change="fetchItems" />
      </div>
    </div>
...
async function locateInventoryPage(targetId) {
  try {
    const res = await api.get('/inventory', {
      params: { keyword: searchKeyword.value, sort_by: sortBy.value, sort_order: sortOrder.value, page: 1, limit: 100000 }
    });
    const allItems = (res.data && res.data.data !== undefined) ? res.data.data : res.data;
    const idx = allItems.findIndex(x => x.id === targetId);
    if (idx !== -1) {
      page.value = Math.floor(idx / limit.value) + 1;
    } else {
...
:global(@keyframes row-flash) {
  0%, 100% { box-shadow: inset 0 0 0 2000px transparent; }
  25%, 75% { box-shadow: inset 0 0 0 2000px rgba(64, 158, 255, 0.22), 0 0 8px rgba(64, 158, 255, 0.2); }
}
